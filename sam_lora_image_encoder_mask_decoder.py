...
class AttentionBlock:
    def __init__(self, layer_idx, block_type, attention_type):
        self.layer_idx = layer_idx
        self.block_type = block_type  # 'self' or 'cross'
        self.attention_type = attention_type  # 'q', 'k', or 'v'

def get_attention_blocks(sam_model):
    attention_blocks = []
    
    # Iterate through image encoder blocks
    for layer_idx, block in enumerate(sam_model.image_encoder.blocks):
        # Add self-attention blocks
        attention_blocks.append(AttentionBlock(layer_idx, 'self', 'q'))
        attention_blocks.append(AttentionBlock(layer_idx, 'self', 'k'))
        attention_blocks.append(AttentionBlock(layer_idx, 'self', 'v'))
        
        # Add cross-attention blocks
        attention_blocks.append(AttentionBlock(layer_idx, 'cross', 'q'))
        attention_blocks.append(AttentionBlock(layer_idx, 'cross', 'v'))
    
    # Iterate through mask decoder transformer layers
    for layer_idx, block in enumerate(sam_model.mask_decoder.transformer.layers):
        # Add self-attention blocks
        attention_blocks.append(AttentionBlock(layer_idx, 'self', 'q'))
        attention_blocks.append(AttentionBlock(layer_idx, 'self', 'v'))
        
        # Add cross-attention blocks
        attention_blocks.append(AttentionBlock(layer_idx, 'cross_ti', 'q'))
        attention_blocks.append(AttentionBlock(layer_idx, 'cross_ti', 'v'))
        attention_blocks.append(AttentionBlock(layer_idx, 'cross_it', 'q'))
        attention_blocks.append(AttentionBlock(layer_idx, 'cross_it', 'v'))
    
    # Add final attention token to image block
    attention_blocks.append(AttentionBlock('final', 'cross_ti', 'q'))
    attention_blocks.append(AttentionBlock('final', 'cross_ti', 'v'))
    
    return attention_blocks

# Example usage:
# sam = sam_model_registry["vit_b"](checkpoint="sam_vit_b_01ec64.pth")
# attention_blocks = get_attention_blocks(sam)
# for block in attention_blocks:
#     print(f"Layer: {block.layer_idx}, Block Type: {block.block_type}, Attention Type: {block.attention_type}")
